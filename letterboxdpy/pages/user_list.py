...
class UserList:
    ...
    def get_title(self) -> str:
        return extract_title(self.dom)

    def get_author(self) -> str:
        return extract_author(self.dom)

    def get_description(self) -> str:
        return extract_description(self.dom)

    def get_date_created(self) -> list:
        return extract_date_created(self.dom)

    def get_date_updated(self) -> list:
        return extract_date_updated(self.dom)

    def get_tags(self) -> list:
        return extract_tags(self.dom)

    def get_movies(self) -> dict:
        return extract_movies(self.url, self.LIST_ITEMS_PER_PAGE)

    def get_count(self) -> int:
        return extract_count(self.dom)

    def get_list_id(self) -> str | None:
        return extract_list_id(self.dom)

    def get_list_meta(self, url: str) -> ListMetaData:
        return extract_list_meta(self.dom, url)
...
def extract_count(dom) -> int:
    ...
    try:
        ...
        if count is None:
            raise ValueError(
                "Meta description not found or missing 'content' attribute."
            )
        return count
...
def extract_list_meta(dom, url: str) -> ListMetaData:
    ...
    try:
        ...
        data["error"] = f"Missing required metadata: {e!s}"
        print(f"Metadata extraction error: {e}")
    except Exception as e:
        data["error"] = f"Unexpected error: {e!s}"
