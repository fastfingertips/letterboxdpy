        ...
        sections = [
            {
                ...
                "data": stats["details"]["followback"],
            },
            {
                "id": "fans",
                "title": "Fans (Reviewers)",
                "data": stats["details"]["fans"],
            },
            {
                "id": "notback",
                "title": "Not Following Back",
                "data": stats["details"]["not_followback"],
            },
            {
                "id": "all_following",
                "title": "Following",
                "data": stats["details"]["all_following"],
            },
            {
                "id": "all_followers",
                "title": "Followers",
                "data": stats["details"]["all_followers"],
            },
        ]

        for section in sections:
            section["ratio_label"] = self._calculate_ratio_info(
                section["id"], len(section["data"]), stats
            )
...
class FollowStatsAnalyzer:
    ...
    def analyze(self):
        ...
        with Progress(
            ...
            progress.add_task(
                description=f"[bold blue]Fetching[/bold blue] followers and following for [cyan]@{self.username}[/cyan]...",
                total=None,
            )
        ...
        self.console.print(
            f"[bold green][OK][/bold green] Data fetched successfully: [white]{len(followers)}[/white] followers, [white]{len(following)}[/white] following"
        )
    ...
    def _calculate_stats(self, following: dict, followers: dict) -> dict:
        ...
        return {
            "summary": {
                ...
                "fans_count": len(fans),
                "followback_ratio": round(
                    len(followback) / len(following_list) * 100, 2
                )
                if following_list
                else 0,
            },
    ...
    def _save(self, stats: dict):
        ...
        summary_grid.add_row(
            "Mutual Follows:",
            f" [bold green]{stats['summary']['mutual_follows']}[/bold green]",
        )
        summary_grid.add_row(
            "Fans (Reviewers):",
            f" [bold yellow]{stats['summary']['fans_count']}[/bold yellow]",
        )
        summary_grid.add_row(
            "Not Following Back:",
            f" [bold red]{stats['summary']['not_followback_count']}[/bold red]",
        )
        summary_grid.add_row(
            "Success Rate:",
            f" [bold blue]{stats['summary']['followback_ratio']}%[/bold blue]",
        )
