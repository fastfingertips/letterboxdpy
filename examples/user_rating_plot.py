...
class LetterboxdRatingPlotter:
    ...
    def _draw_header_and_stats(
        self, axis, stats_axis, total_count: int, average: float, most_given: float
    ) -> None:
    ...
    def _label_bars(self, axis, bars_, counts_) -> None:
        ...
        for bar, count in zip(bars_, counts_, strict=False):
            if count == max_count:
    ...
    def create_plot(self, ratings: dict):
        ...
        rating_counts = np.array(
            [ratings.get(rating, 0) for rating in rating_positions]
        )
        ...
        bars = ax.bar(
            rating_positions, rating_counts, width=0.45, color=Colors.BLUE, alpha=0.85
        )

        self._draw_header_and_stats(
            ax, ax_stats, total_ratings, average_rating, most_given_rating
        )
