...
class LetterboxdStatisticsPlotter:
    ...
    def plot_statistics(self) -> None:
        ...
        years_range = (
            f"{min(self.stats_by_year.keys())}-{max(self.stats_by_year.keys())}"
        )

        # Safe window title access
        if fig.canvas.manager:
            fig.canvas.manager.set_window_title(
                f"Letterboxd Statistics - {self.username} ({years_range})"
            )

        fig.suptitle(
            f"{self.username} - Movies Watched ({years_range})",
            fontsize=16,
            color="white",
        )
        ...
        for i, (year, stats) in enumerate(self.stats_by_year.items()):
            ...
            ax_monthly.bar(
                months_labels, monthly_values, color=Colors.GREEN, alpha=0.85
            )
    ...
    def run(self):
        ...
        parser.add_argument(
            ...
            help=f"Start year (default: {current_year - 1})",
        )
        parser.add_argument(
            "--end-year",
            type=int,
