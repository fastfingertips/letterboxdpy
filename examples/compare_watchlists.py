...
class WatchlistHtmlRenderer:
    ...
    def __init__(
        self, username: str, my_films: dict, film_users: dict, user_totals: dict
    ):
    ...
    def _prepare_users_data(self) -> list:
        ...
        sorted_user_list = sorted(
            user_matches.items(), key=lambda x: len(x[1]), reverse=True
        )
...
class WatchlistComparator:
    ...
    def fetch_user_watchlist(self) -> bool:
        ...
        with Progress(
            *self.PROGRESS_COLUMNS, console=self.console, transient=True
        ) as progress:
            progress.add_task(
                description=f"[bold blue]Fetching[/bold blue] watchlist for [cyan]@{self.username}[/cyan]...",
                total=None,
            )
            # use self.force_refresh to allow caching the user's own watchlist
            data, cache_info = self._get_watchlist(
                self.username, force_refresh=self.force_refresh
            )
    ...
    def fetch_following(self):
        ...
        with Progress(
            *self.PROGRESS_COLUMNS, console=self.console, transient=True
            ...
            progress.add_task(
                description=f"[bold blue]Fetching[/bold blue] following list for [cyan]@{self.username}[/cyan]...",
                total=None,
            )
        ...
        self.console.print(
            f"[bold green][OK][/bold green] Following fetched: [white]{len(self.following)}[/white] users"
        )
            ...
            *self.PROGRESS_COLUMNS,
            ...
            for i, user in enumerate(self.following, 1):
                self._process_single_user(
                    progress, task, user, i, total_following, idx_width
                )
                progress.advance(task)

            progress.update(
                task, description="[bold green]All users processed[/bold green]"
            )
    ...
    def _process_single_user(self, progress, task, user, index, total, idx_width):
        ...
        progress.update(
            task,
            description=f"Processing [{self.STYLE_SECONDARY}]@{user}[/{self.STYLE_SECONDARY}]",
        )
        ...
        if their_ids:
            ...
            stats_str = (
                f"{shared_count:>4}/{total_count:<5} common ({percentage:>5.1f}%)"
            )
    ...
    def display_recommendations(self):
        ...
        table.add_column(
            "Matches",
            justify="center",
            footer=Text(f"{len(self.film_popularity)} films", style=self.STYLE_PRIMARY),
        )
        ...
        for i, (film_id, count) in enumerate(self.film_popularity.most_common(20), 1):
            ...
            table.add_row(
                str(i), str(count), self._format_film_title(film_id), users_display
            )
    ...
    def save_results(self):
        ...
        # Save HTML
        renderer = WatchlistHtmlRenderer(
            self.username, self.my_films, self.film_users, self.user_total_counts
        )
        stats = {
        ...
        summary_grid.add_row(
            "Common Films Found:",
            f" [bold green]{len(self.film_popularity)}[/bold green]",
        )
        summary_grid.add_row(
            "Scanned Users:", f" [bold yellow]{len(self.following)}[/bold yellow]"
        )
        summary_grid.add_row("JSON Export:", f" [dim]{base_path}.json[/dim]")
        summary_grid.add_row(
            "HTML Report:", f" [bold blue]{base_path}.html[/bold blue]"
        )
...
if __name__ == "__main__":
    ...
    if not comparator.fetch_user_watchlist():
        comparator.console.print(
            "[bold red]Error:[/bold red] User's watchlist is empty or private."
        )
        sys.exit(1)
